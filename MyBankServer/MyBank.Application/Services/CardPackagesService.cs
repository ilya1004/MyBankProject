...
public class CardPackagesService : ICardPackagesService
{
    ...
    public async Task<ServiceResponse<CardPackage>> GetById(int id)
    {
        ...
        if (cardPackage == null)
        {
            return new ServiceResponse<CardPackage>
            {
                Status = false,
                Message = $"Card package with given id ({id}) not found",
                Data = default
            };
        }
    ...
    public async Task<ServiceResponse<bool>> UpdateInfo(
        int id,
        string name,
        decimal price,
    ...
    {
        var status = await _cardPackagesRepository.UpdateInfo(
            id,
            name,
            price,
            operationsNumber,
