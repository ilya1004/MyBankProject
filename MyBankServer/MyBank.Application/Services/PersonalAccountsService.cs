...
public class PersonalAccountsService : IPersonalAccountsService
{
    ...
    public PersonalAccountsService(IPersonalAccountsRepository personalAccountsRepository, ICardsRepository cardsRepository, ITransactionsRepository transactionsRepository)
    {
    ...
    public async Task<ServiceResponse<bool>> UpdateTransfersStatus(int id, bool isForTransfersByNickname)
    {
        ...
        if (status == false)
        {
            return new ServiceResponse<bool> { Status = false, Message = $"Personal account with given id ({id}) not found", Data = default };
        }
    ...
    public async Task<ServiceResponse<bool>> CloseAccount(int id)
    {
        ...
        if (personalAccount.CurrentBalance != 0)
            return new ServiceResponse<bool> { Status = false, Message = $"Personal account must have zero current balance to delete it", Data = default };
    ...
    public async Task<ServiceResponse<bool>> MakeTransfer(int personalAccountId, string accountSenderNumber, string userSenderNickname, 
        string? accountRecipientNumber, string? cardRecipientNumber, string? userRecipientNickname, decimal amount)
    {
        ...
        {
            status = await _personalAccountsRepository.UpdateBalanceDelta(accountRecipientNumber, amount);

            if (status == false)
                return new ServiceResponse<bool> { Status = false, Message = $"Unknown error. Maybe personal account with given number ({accountRecipientNumber}) not found", Data = default };
            ...
            await _transactionsRepository.Add(
                new Transaction(0, amount, DateTime.UtcNow, true, "Перевод средств по номеру счета",
                accountSenderNumber, userSenderNickname, accountRecipientNumber, accountRecipient.UserOwner!.Nickname));
        }
        else if (cardRecipientNumber != null)
        {
            ...
            if (card == null)
                return new ServiceResponse<bool> { Status = false, Message = $"Unknown error. Maybe card with given number ({cardRecipientNumber}) not found", Data = default };

            status = await _personalAccountsRepository.UpdateBalanceDelta(card.PersonalAccount!.Number, amount);

            if (status == false)
                return new ServiceResponse<bool> { Status = false, Message = $"Unknown error. Maybe personal account with given number ({card.PersonalAccount!.Number}) not found", Data = default };

            await _transactionsRepository.Add(
                new Transaction(0, amount, DateTime.UtcNow, true, "Перевод средств по номеру карты",
                accountSenderNumber, userSenderNickname, card.PersonalAccount!.Number, card.User!.Nickname));
        }
        else if (userRecipientNickname != null)
        {
            var personalAccount = await _personalAccountsRepository.GetIsForTransfersByNickname(userRecipientNickname);

            if (personalAccount == null)
                return new ServiceResponse<bool> { Status = false, Message = $"Неизвестная ошибка.", Data = default };

            status = await _personalAccountsRepository.UpdateBalanceDelta(personalAccount.Id, amount);

            if (status == false)
                return new ServiceResponse<bool> { Status = false, Message = $"Unknown error. Maybe personal account with given id ({personalAccount.Id}) not found", Data = default };

            await _transactionsRepository.Add(
                new Transaction(0, amount, DateTime.UtcNow, true, "Перевод средств по номеру карты",
                accountSenderNumber, userSenderNickname, personalAccount.Number, userRecipientNickname));
        }
