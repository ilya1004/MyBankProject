...
public class DepositAccountsRepository : IDepositAccountsRepository
{
    ...
    public async Task<DepositAccount> GetById(int id, bool withUser)
    {
        ...
        if (withUser)
        {
            depositAccountEntity = await _dbContext
                .DepositAccounts.AsNoTracking()
                .Include(da => da.User)
    ...
    {
        var depositAccountEntitiesList = await _dbContext
            .DepositAccounts.AsNoTracking()
    ...
    public async Task<bool> UpdateClosingInfo(
        int id,
        int newBalance,
        DateTime dateTime,
        bool isActive
