...
const getCreditsData = async () => {
  ...
  try {
    const res = await axiosInstance.get(
      `CreditAccounts/GetAllInfo?includeData=${true}&onlyActive=${true}`
    );
...
export default function ModeratorCreditsPage() {
    ...
    return (
      ...
      >
        ...
        <Column
          ...
          key="creationDate"
          render={(_, record) => (
            <Text>{convertDate(record.creationDate)}</Text>
          )}
        />
        <Column
          width="180px"
      ...
      >
        <Flex style={{ width: "85%" }}>
          <Title style={{ marginLeft: "40px" }} level={2}>
            Список всех кредитов пользователей
          </Title>
        </Flex>
        <Flex align="center" justify="center" style={{ width: "100%" }}>
          ...
          >
            ...
            <Column
              ...
              key="user"
              render={(user, record) => (
                <Link to={`/moderator/users/${user.id}`}>
                  {`${user.nickname} (${user.surname} ${user.name} ${user.patronymic})`}
                </Link>
              )}
            />
            <Column
